...
const sendMail = async(req, res, next) => {
    ...
    transport.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          return res.status(HTTP_STATUS.OK).json({"success": false, "messageid" : "" , "error" : error.message  });
        }
        else{
...
try {
    ...
    transport.verify(function(error, success) {
        if (error) {
            return res.status(HTTP_STATUS.OK).json({"available" : false , "message" : "Server is not ready"});
        } else {
            return res.status(HTTP_STATUS.OK).json({"available" : true , "message" : "Server is ready to take our messages"});
        }
      });
